	...
	<div id="container">  
	 	<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
	    	...
	    	<text font-family="monospace" font-size="10" fill="#1d1f20">
	    		<textPath id="text" xlink:href="#s">{{ text }}</textPath>
	    	</text>  
	  	</svg> 
...
<script>
	import { getBreakdown, createNewRandomBreakdown } from './breakdownController.js';
	    	...
	    	getBreakdown(){
                ...
                getBreakdown().then(response => {
                    ...
                    response.data.randomBreakdowns.map(function(value,key) {
                        self.breakDownTable.push(`${value.values} `);
                    });
